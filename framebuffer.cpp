...
FrameBuffer::FrameBuffer(int w, int h)
{
  ...
  // if an error occures and it returns false, exit.
  if (!this->framebuffer)
  {
    exit(-2);
  }
...
{
  // checks for error coords
  if ((x<0)||(x>=this->width)||(y<0)||(y>=this->height))
  {
    return -1;
  }

  // if red has a value more than 1 or less than 0, error.
  if ((red > 1.0f) || (red < 0.0f)) cerr<<"out of range\n";
  
  // otherwise, initialise the pixels rgb
